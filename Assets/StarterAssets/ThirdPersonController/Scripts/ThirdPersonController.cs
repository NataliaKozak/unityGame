...
namespace StarterAssets
{
	...
	public class ThirdPersonController : MonoBehaviour
	{
        ...
        {
            if(other.gameObject.name == "триггерВхода")
            {
				

                if (!MainManager.Inventory.CheckItems())
                {
					MainManager.Messenger.WriteMessage("Для перехода в эту комнату соберите дневник, блокнот и улику.");
				}
                else
                {
					int index = SceneManager.GetActiveScene().buildIndex; // берем индекс запущенной сцены
					index = (index == 1) ? 2 : 1; // меняем индекс с 0 на 1 или с 1 на 0
					StartCoroutine(AsyncLoad(index)); // запускаем асинхронную загрузку сцены
				}
				
			}

			if (other.gameObject.name == "выход")
			{

					int index = SceneManager.GetActiveScene().buildIndex; // берем индекс запущенной сцены
					index = (index == 4) ? 5 : 4; // меняем индекс с 0 на 1 или с 1 на 0
					StartCoroutine(AsyncLoad(index)); // запускаем асинхронную загрузку сцены
